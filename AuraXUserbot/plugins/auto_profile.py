...
from AuraXUserbot import ALIVE_NAME, BIO_MSG, CMD_HELP
...
async def _(event):
    if event.fwd_from:
        return
    AuraX = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    ...
    await AuraX.edit(f"Auto Name has been started my Master")
