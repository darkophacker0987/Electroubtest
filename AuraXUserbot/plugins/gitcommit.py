...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of AuraXBot`")
        return
    AuraXBot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await AuraXBot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, AuraXBot)


async def git_commit(file_name, AuraXBot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await AuraXBot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await AuraXBot.edit("Cannot Upload Plugin")
    else:
        return await AuraXBot.edit("`Committed Suicide`")
