...
async def _(event):
    ...
    AuraXevent = await edit_or_reply(event, "Downloading to my local, for analysis  🙇")
    if event.reply_to_msg_id:
        ...
        if (
            ...
            await AuraXevent.edit("Starting analysis, using IBM WatSon Speech To Text")
            headers = {
            ...
            if "results" in r:
                ...
                await AuraXevent.edit(string_to_show)
            else:
                await AuraXevent.edit(r["error"])
            # now, remove the temporary file
            os.remove(required_file_name)
    else:
        await AuraXevent.edit("Reply to a voice message, to get the relevant transcript.")
